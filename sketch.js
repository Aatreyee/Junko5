...
var snowScene,snowSceneImg;

var snowball,snowballGroup,snowballImg;
var TimeLeft=500;                    

var snowGirl,snowGirlImg;

var flag,flagImg;

var log,log2,log3,log4,log5,log6,log7,log8,log9,log10,log11,log12;
...
function preload(){
  ...
  //Loading all the images
    ...
    snowSceneImg = loadImage("images/snowScene.jpg");

    snowballImg = loadImage("images/snowBall.png");

    snowGirlImg = loadImage("images/snowGirl.jpg");
    
    flagImg = loadImage("images/victoryFlag.png");

    lastbImg = loadImage("images/lastM.jpg");
}
function setup(){
  ...
  //Creating all the the sprites and adding their images
   ...
   snowScene = createSprite(width/2,height/2);
   snowScene.addImage(snowSceneImg);

   yakScene = createSprite(width/2,height/2);
   yakScene.addImage(yakSceneImg);
   yakScene.visible = false;

   lastb = createSprite(width/2,height/2);
   lastb.addImage(lastbImg);
   lastb.visible = false;
   ...
   flag = createSprite(200,height-300);
   ...
   snowGirl = createSprite(width/2+400,height/2);
   snowGirl.addImage(snowGirlImg);
   snowGirl.scale=0.2;
   snowGirl.visible = false;
...
for(var k=0;k<hotDog.length;k++){
  ...
  if(mousePressedOver(button1)){
    ...
    text("Time Left :"+TimeLeft,width/2+100,height/2);
      ...
      textSize(30);
      textFont("curly");
      text("GOOD JOB!!.You have collected enough money for the climb.Ready for the next challenge?",200,100);
      text("The next challenge is ....YAK ATTACK. Yes, you have to survive this attack on the way ",200,200);
      text("to Everest and then you will get a GUIDE BOOK TO MT. EVEREST",200,300);
  }
    ...
    background(snowSceneImg);
    survivalTime=survivalTime-1;
    dollarGroup.destroyEach();
    snowGirl.visible=true;
    cyclist.destroy();
    yakJunko.destroy();
    nailGroup.destroyEach();
    yakGroup.destroyEach();
    if(keyDown(UP_ARROW)){
      snowGirl.y = snowGirl.y-10;
    }
    ...
    //if(snowGirl.isTouching(snowballGroup)){
     // gameState=5
   // }
    ...
    text("Survival Time :"+survivalTime,width/2,height/2);
  }
...
function spawnYak(){
  if (frameCount % 50 === 0){
    ...
    yak.velocityX =12;
    yak.y = Math.round(random(0,height));
...
function spawnSnowball(){
  if (frameCount % 10 === 0){
    var snowball = createSprite(400,56,78,56);
    snowball.addImage(snowballImg);
    
    snowball.velocityY = +20;
    
    snowball.x = Math.round(random(0,width));
    snowball.scale=0.6;
    snowball.lifetime = 1000;
